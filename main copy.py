...
from models.p2vnet import P2VNet
...
from models.siamunet_ef_deepsup import SiamUNet_EF_deepsup
from models.siamunet_ef_tfblock import SiamUNet_EF_tfblock
...
def get_args():
    ...
    parser.add_argument('-c', '--config-name', type=str, help='YAML Config name', 
                        dest='CONFIG', default='CD_changeformer_emb128')
...
def main_train(args, exp_config, out_file):
    ...
    if 'mtbit' in exp_config['model']['model']:
        ...
        model = SiamUNet_EF()
    elif exp_config['model']['model']=='siamunet_ef_deepsup':
        model = SiamUNet_EF_deepsup()
    elif exp_config['model']['model']=='siamunet_ef_tfblock':
        model = SiamUNet_EF_tfblock()
    ...
    if args.eval!=True:
        ...
        trainer.fit(model=pl_model, train_dataloaders=train_loader#, val_dataloaders=valid_loader,
                    #,ckpt_path='results/CD_changeformer_emb128/lightning_logs/version_0/checkpoints/epoch=17-step=4590.ckpt'
                   )
    else:
