...
class DecoderTransformer_v3(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True,
        ...
        self.dense_2x = cgblock(self.embedding_dim, self.embedding_dim,
                              dilation=2, reduction=8, skip_connect=False)#nn.Sequential(ResidualBlock(self.embedding_dim))
        self.dense_2x_3d = cgblock(self.embedding_dim, self.embedding_dim,
                              dilation=2, reduction=8, skip_connect=False)
        self.convd1x = UpsampleConvLayer(self.embedding_dim, self.embedding_dim, kernel_size=4, stride=2)
        self.dense_1x = cgblock(self.embedding_dim, self.embedding_dim,
                              dilation=2, reduction=8, skip_connect=False)#nn.Sequential(ResidualBlock(self.embedding_dim))
        self.dense_1x_3d = cgblock(self.embedding_dim, self.embedding_dim,
                               dilation=1, reduction=4, skip_connect=False)
        self.change_probability = ConvLayer(self.embedding_dim, self.output_nc, kernel_size=3, stride=1, padding=1)
        self.change_3d = ConvLayer(self.embedding_dim, self.output_nc, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs1, inputs2):
        ...
        # Residual block
        x = self.dense_2x(x)
        x3d = self.dense_2x_3d(x3d)
        # Upsampling x2 (x1 scale)
        x = self.convd1x(x)
        x3d = self.convd1x(x3d)
        # Residual block
        x = self.dense_1x(x)
        x3d = self.dense_1x_3d(x3d)

        # Final prediction
        cp = self.change_probability(x)
        cp_3d = self.change_3d(x3d)
        # outputs.append(cp)
...
# ChangeFormerV6:
class hmcdnet_dchead_levir(nn.Module):

    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=64):
        super(hmcdnet_dchead_levir, self).__init__()
        # Transformer Encoder
