...
class EncoderTransformer_x2(nn.Module):
    def __init__(self, img_size=512, patch_size=3, in_chans=3, num_classes=2, embed_dims=[32, 64, 128, 256, 512],
...
class ChangeFormerV4(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False):
        ...
        self.Tenc_x2    = EncoderTransformer_x2(img_size=512, patch_size=3, in_chans=input_nc, num_classes=output_nc, embed_dims=self.embed_dims,
...
class DecoderTransformer_v3(nn.Module):
    ...
    """
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
                    in_channels = [32, 64, 128, 256], embedding_dim= 64, output_nc=4, 
        ...
        self.change_probability = ConvLayer(self.embedding_dim, self.output_nc, kernel_size=3, stride=1, padding=1)
        self.change_3d = ConvLayer(self.embedding_dim, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs1, inputs2):
        ...
        #Final prediction
        cp = self.change_probability(x)
        cp_3d = self.change_3d(x)
        
        cp_3d = self.activate_3d(cp_3d)
...
class ChangeFormerV5(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.Tenc_x2    = EncoderTransformer_v3(img_size=512, patch_size = 4, in_chans=input_nc, num_classes=output_nc, embed_dims=self.embed_dims,
...
class ChangeFormerV6(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=4, decoder_softmax=False, embed_dim=64):
