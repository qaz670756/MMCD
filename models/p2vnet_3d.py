    ...
    def forward(self, x1, x2, add_feats=None):
        ...
        for i in range(self.n_layers):
            ...
            if i > 0 and add_feats is not None:
                add_feat = nn.functional.interpolate(add_feats[i-1], size=x.shape[2:])
...
class SimpleDecoder(nn.Module):
    def __init__(self, itm_ch, enc_chs, dec_chs, out_ch):
        ...
        self.conv_out_3d = Conv1x1(dec_chs[-1], 1)
    ...
    def forward(self, x, feats):
        ...
        y_3d = self.conv_out_3d(x)
        y_3d = self.activate_3d(y_3d)

        return y_3d


class p2vnet_3d(nn.Module):
    def __init__(self, in_ch=3, out_ch=3, video_len=8, enc_chs_p=(32,64,128), enc_chs_v=(64,128), dec_chs=(256,128,64,32)):
        super().__init__()
        if video_len < 2:
            raise ValueError
        ...
        self.decoder = SimpleDecoder(enc_chs_p[-1], (2*in_ch,)+enc_chs_p, dec_chs,out_ch)
    ...
    def forward(self, t1, t2, return_aux=True):
        ...
        pred_3d = self.decoder(feats_p[-1], feats_p)


        if return_aux:
            pred_v = self.conv_out_v(feats_v[-1])
            pred_v = F.interpolate(pred_v, size=pred_3d.shape[2:])
            return [pred_3d, pred_v]
        else:
            return pred_3d
