...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, B, H, W):
        ...
        dim_t = self.temperature ** (2 * (torch.div(dim_t, 2.0,rounding_mode='floor')) / self.hidden_dim)
...
class LiteMono(nn.Module):
    ...
    def __init__(self, in_chans=3, model='lite-mono', height=192, width=640,
        ...
        ConvBlock(96, 96//2),ConvBlock(160, 160//2),ConvBlock(256, 256//2)
        ])
    ...
    def forward(self, x1,x2):
        ...
        features_x1 = [_[:bsize,:,:,:] for _ in x]
        features_x2 = [_[bsize:,:,:,:] for _ in x]
        
        x = [torch.cat([f1,f2],dim=1) for f1,f2 in zip(features_x1,features_x2)]
        #import pdb;pdb.set_trace()
        x = [self.lateral_convs[i](x[i]) for i in range(len(x))]
        
        out_2d, out_3d = self.decoder(x)
...
class DepthDecoder(nn.Module):
    def __init__(self, num_ch_enc=np.array([48,80,128]), scales=range(3), 
                 num_output_channels=1, use_skips=True,num_classes=2):
        super().__init__()
        ...
        for i in range(2, -1, -1):
            ...
            #self.convs[("upconv", i, 1)] = ConvBlock(num_ch_in, num_ch_out)
            #self.convs[("upconv_2d", i, 1)] = ConvBlock(num_ch_in, num_ch_out)

        for s in self.scales:
            self.convs[("dispconv", s)] = nn.Sequential(
                Conv3x3(self.num_ch_dec[s], self.num_ch_dec[s]//2),
                nn.BatchNorm2d(self.num_ch_dec[s]//2, eps=1e-5),
                Conv3x3(self.num_ch_dec[s]//2, 1),
                nn.Tanh()
            )
            self.convs[("dispconv_2d", s)] = nn.Sequential(
                Conv3x3(self.num_ch_dec[s], self.num_ch_dec[s]//2),
                nn.BatchNorm2d(self.num_ch_dec[s]//2, eps=1e-5),
                Conv3x3(self.num_ch_dec[s]//2, num_classes),
            )
    ...
    def forward(self, input_features):
        ...
        for i in range(2, -1, -1):
            ...
            #f_2d = self.convs[("upconv_2d", i, 1)](x)

            if i in self.scales:
                #import pdb;pdb.set_trace()
                f_3d = upsample(self.convs[("dispconv", i)](x), mode='bilinear')
                f_2d = upsample(self.convs[("dispconv_2d", i)](x), mode='bilinear')
                self.outputs.insert(0,f_3d)
                self.outputs_2d.insert(0,f_2d)
                

        return self.outputs_2d, self.outputs
