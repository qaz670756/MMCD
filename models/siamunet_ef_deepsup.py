...
class SiamUNet_EF_deepsup(nn.Module):
    def __init__(self, in_ch=3, out_ch=2, use_dropout=False):
        ...
        # out3-128*128
        self.out3_convs = nn.Sequential(
        Conv3x3(32, 16, norm=True, act=True),self.make_dropout(),
        Conv3x3(16, out_ch))

        self.out3_convs_3d = nn.Sequential(
        Conv3x3(32, 16, norm=True, act=True),self.make_dropout(),
        Conv3x3(16, 1),nn.Tanh())
        
        # out2-256*256
        self.out2_convs = nn.Sequential(
        Conv3x3(16, 8, norm=True, act=True),self.make_dropout(),
        Conv3x3(8, out_ch))

        self.out2_convs_3d = nn.Sequential(
        Conv3x3(16, 8, norm=True, act=True),self.make_dropout(),
        Conv3x3(8, 1),nn.Tanh())
        
        # out1-512*512
        self.out1_convs = nn.Sequential(
        Conv3x3(32, 16, norm=True, act=True),self.make_dropout(),
        Conv3x3(16, out_ch))

        self.out1_convs_3d = nn.Sequential(
        Conv3x3(32, 16, norm=True, act=True),self.make_dropout(),
        Conv3x3(16, 1),nn.Tanh())
    ...
    def forward(self, t1, t2):
        ...
        x4d = torch.cat([F.pad(x4d, pad=pad4, mode='replicate'), x43_1], 1)
        ...
        out1 = self.out1_convs(x1d)
        out1_3d = self.out1_convs_3d(x1d)
        
        return [out1,out2,out3], [out1_3d,out2_3d,out3_3d]
