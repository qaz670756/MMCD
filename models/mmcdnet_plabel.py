...
class DecoderTransformer_v3(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c1 = self.diff_c1(torch.cat(self.CFM_c1(_c1_1, _c1_2), dim=1)) + F.interpolate(_c2, scale_factor=2,
                                                                                        mode="bilinear")
        _,C,H,W = _c2_1.shape
        #vis_feature = torch.cat([_c2_1.view(C,H*W), _c2_2.view(C,H*W)], dim=1).clone().detach()
        ...
        xplabel = self.change_plabel(x+self.soft_thresh(x3d,x))
        # outputs.append(cp)
