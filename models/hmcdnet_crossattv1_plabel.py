...
class EncoderTransformer_v3(nn.Module):
    ...
    def forward(self, x):
        x = self.forward_features(x)
        return x
class appro_thresh_func_square(nn.Module):
    def __init__(self, t=1.0):
        super().__init__()
        self.sigmoid = nn.Sigmoid()
        self.t = t
    def forward(self, x):
        x = 2 * self.sigmoid(x / self.t) - 1
        return torch.pow(x,2)

class appro_thresh_func(nn.Module):
    def __init__(self, t=2.0):
        super().__init__()
        self.sigmoid = nn.Sigmoid()
        self.t = t
        self.min_scale = -27.291
        self.max_scale = 83.266

    def forward(self, x):
        x = (x + 1) * (self.max_scale - self.min_scale) / 2 + self.min_scale
        x = 2 * self.sigmoid(x / self.t) - 1
        return torch.pow(x, 2)

class SoftThresh_squresigmoid(nn.Module):
    def __init__(self, in_features=48, hidden_features=64, out_features=1, act_layer=nn.GELU, drop=0.1):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features

        self.thresh_func = appro_thresh_func_square()
        self.lateral2d = nn.Sequential(nn.Conv2d(in_channels=in_features,out_channels=in_features,kernel_size=1),
                                       nn.BatchNorm2d(num_features=in_features))

        # self.mlp_out = nn.Sequential(nn.Conv2d(in_channels=in_features,out_channels=in_features,kernel_size=1),
        #                                nn.BatchNorm2d(num_features=in_features))
    ...
    def forward(self, x, y):
        #B,C,H,W = x.shape
        x = self.thresh_func(x)
        #x = self.lateral3d(x)
        y = self.lateral2d(y)
        #y = self.relu(x + y)
        #y = y.softmax(dim=1)

        return self.mlp_out(y*x)
...
class SoftThresh(nn.Module):
    def __init__(self, in_features=48, hidden_features=64, out_features=1, act_layer=nn.GELU, drop=0.1):
        ...
        self.fc2 = nn.Linear(hidden_features, out_features)
        self.drop = nn.Dropout(drop)
        self.thresh_func = appro_thresh_func()
        # self.lateral2d = nn.Sequential(nn.Conv2d(in_channels=in_features,out_channels=in_features,kernel_size=1),
        #                                nn.BatchNorm2d(num_features=in_features))
        #
        # self.mlp_out = nn.Sequential(nn.Conv2d(in_channels=in_features,out_channels=in_features,kernel_size=1),
...
class SoftThresh_conv2dout(nn.Module):
    def __init__(self, in_features=48, hidden_features=64, out_features=1, act_layer=nn.GELU, drop=0.1):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
...
class DecoderTransformer_v3(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True,
        ...
        self.CFM_c4 = crossAttBlock(dim=embed_dims[0], num_head=num_heads[0], mlp_ratio=mlp_ratios[0],
                                    sr_ratio=sr_ratios[0], in_chans=self.embedding_dim)
        self.diff_c4 = conv_diff(in_channels=2 * self.embedding_dim, out_channels=self.embedding_dim)

        self.CFM_c3 = crossAttBlock(dim=embed_dims[1], num_head=num_heads[1],
                                    mlp_ratio=mlp_ratios[1], sr_ratio=sr_ratios[1], in_chans=self.embedding_dim)
        self.diff_c3 = conv_diff(in_channels=2 * self.embedding_dim, out_channels=self.embedding_dim)

        self.CFM_c2 = crossAttBlock(dim=embed_dims[2], num_head=num_heads[2],
                                    mlp_ratio=mlp_ratios[2], sr_ratio=sr_ratios[2], in_chans=self.embedding_dim)
        self.diff_c2 = conv_diff(in_channels=2 * self.embedding_dim, out_channels=self.embedding_dim)

        self.CFM_c1 = crossAttBlock(dim=embed_dims[1], num_head=num_heads[3],
                                    mlp_ratio=mlp_ratios[3], sr_ratio=sr_ratios[3], in_chans=self.embedding_dim)
        self.diff_c1 = conv_diff(in_channels=2 * self.embedding_dim, out_channels=self.embedding_dim)
        self.soft_thresh = SoftThresh(in_features=self.embedding_dim, hidden_features=64, out_features=self.embedding_dim)
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c4 = self.diff_c4(torch.cat(self.CFM_c4(_c4_1, _c4_2), dim=1))
        ...
        _c3 = self.diff_c3(torch.cat(self.CFM_c3(_c3_1, _c3_2), dim=1)) + F.interpolate(_c4, scale_factor=2,
                                                                                        mode="bilinear")
        ...
        _c2 = self.diff_c2(torch.cat(self.CFM_c2(_c2_1, _c2_2), dim=1)) + F.interpolate(_c3, scale_factor=2,
                                                                                        mode="bilinear")
        ...
        _c1 = self.diff_c1(torch.cat(self.CFM_c1(_c1_1, _c1_2), dim=1)) + F.interpolate(_c2, scale_factor=2,
                                                                                        mode="bilinear")
        ...
        xplabel = self.change_plabel(x) + self.change_plabel(self.soft_thresh(x3d,x))
        ...
        #return x3d,[x2d,p_c1,p_c2],xplabel
        return x3d, xplabel, x2d, vis_feature


# ChangeFormerV6:
class hmcdnet_crossattv1_plabel(nn.Module):

    def __init__(self, input_nc=3, output_nc=3, decoder_softmax=False, embed_dim=64):
        super(hmcdnet_crossattv1_plabel, self).__init__()
        # Transformer Encoder
