...
def main_train(args, exp_config, out_file):
    ...
    save_ckpt_func= ModelCheckpoint(save_top_k=-1, every_n_epochs=1,save_on_train_epoch_end=True)
    try:
    ...
    if args.eval != True:
        #import pdb;pdb.set_trace()
        logger = pl.loggers.CSVLogger("logs", version=1)
        trainer = pl.Trainer(default_root_dir=out_file,
                             ...
                             callbacks=[save_ckpt_func], precision='16-mixed')
        #import pdb;pdb.set_trace()
        if not resume_path:
